...
public class ReferenceFinderData
{
    ...
    public void CollectDependenciesInfo()
    {
        try
        {
            ...
            for (int i = 0; i < allAssets.Length; i++)
            {
                ...
                if (allAssets[i].Contains("/Editor/Data/UnityExtensions") == false)
                {
    ...
    private void WriteToChache()
    {
        ...
        using (FileStream fs = File.OpenWrite(CACHE_PATH))
        {
            ...
            {
                dependencies = assetDict[guid].dependencies;
                indexes.Clear();
                foreach (var v in dependencies)
                {
                    if (guidIndex.ContainsKey(v))
                        indexes.Add(guidIndex[v]);
                }
                serializedDenpendencies.Add(indexes.ToArray());
            }
