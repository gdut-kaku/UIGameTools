...
namespace KakuEditorTools
{
    ...
    public class GuidRegeneratorWindow : WindowEditorBase<GuidRegeneratorWindow>
    {
        ...
        protected override void OnGUI()
        {
            ...
            EditorGUILayout.LabelField(sourcePath, EditorStyles.textField);
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(targetPath, EditorStyles.textField);
            if (GUILayout.Button("选择文件夹", GUILayout.Width(150)))
            {
                DirectoryInfo directoryInfo = new DirectoryInfo(sourcePath);
                string referencePath = directoryInfo.Parent.FullName;
                string result = EditorUtility.OpenFolderPanel("Select assetfold to change guid.", referencePath, "");
                if (result.Contains(Application.dataPath))
                {
